...
//             2024_03_11 Oscar Gonzalez: Adding some more information about firmware version changing within the same event.
//
...
//   int wheel      = AB7Wheel;
   ...
   if (0 || doHexDumping_) std::cout << "OGDT-INFO: Dump + AMC Header 1, BX: "<<bx<<" event: "<<event<<std::endl;
    ...
    if (fw_version==-1) {
      ...
      edm::LogWarning("oglez_dtab7_unpacker")
        <<"The firmware version is changing depending on the AMC in the same event? Please check "<<fw_version<<" "<<((dataWord>>7)&0x01FF)
        <<" in event: "<<e.run()<<"-"<<e.getLuminosityBlock().luminosityBlock()
        <<"-"<<e.eventAuxiliary().event()<<std::endl;
      fw_version = (dataWord>>7)&0x01FF;  // The changed one is the "reference" now.
    }
...
//OLD   }
  ...
  int uind = 0;
  int uphi = (int) round(phiAngle*cmsdt::PHIRES_CONV);  // phiAngle
  int uphib = (int) round(phiBending*cmsdt::PHIBRES_CONV);  // phiBending
